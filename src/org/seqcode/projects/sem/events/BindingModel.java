...
public class BindingModel {
	public final static int SMOOTHING_SETPSIZE = 30;
			...
			while ((line = reader.readLine()) != null) {
				...
				if(words.length>=2) {
					...
					if ((dist.intValue() >= minDist) && (dist.intValue() <= maxDist)) {
						...
						if (p.cdr().doubleValue()>=0) // shouble be non-negative value
	...
	protected void loadData(List<Pair<Integer, Double>> bindingDist) {
		...
		for(Pair<Integer, Double> p: bindingDist) {
			int index = p.car();
			double val = p.cdr();
			//if list not properly sorted (need to make this into an exception)
			if(index-last<0) {
				System.err.println("Incorrectly sorted bindng read distribution data!");
				System.exit(1);
			}
